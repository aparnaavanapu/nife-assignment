...
const SignupRoute = () => {
  ...
  const signupFailure = (error_msg) =>{
    alert(error_msg);
  }

  const onSubmitForm = async (event ) =>{
    event.preventDefault();
    const url=`${process.env.REACT_APP_DOMAIN}/auth/signup`;
    const userDetails ={
        name:userName,
        email:userMail,
    ...
    try{
        ...
        console.log(data);
        if (response.ok){
            signupSuccess(data.jwt_token);
        
        }
        else{
            signupFailure(data.error);
        }
    }
    catch(error){
        console.error("Fetch error :",error);
        signupFailure("Something went wrong. Plese try again.")
    }
  ...
  return (
    <div className="flex justify-center items-center h-screen bg-white text-gray-800">
      <div className="w-full max-w-[400px] sm:max-w-[450px] m-4  md:w-[400px] p-8 rounded-2xl shadow-xl border border-gray-300 bg-white">
        <h1 className="text-2xl font-semibold text-center text-pink-500 mb-8">Register</h1>
        ...
        <form className="flex flex-col gap-8" onSubmit={onSubmitForm}>
          ...
          <button type="submit" className="w-full mt-4 py-2 border-2 border-pink-500 rounded-full text-pink-500 font-semibold  hover:bg-pink-500 hover:text-white transition-all">Register</button>
          <p className="text-center text-sm text-gray-400">Already have an account?{" "}<span className="text-pink-500 cursor-pointer hover:underline" onClick={gotoLogin}>Login</span></p>
        </form>
      </div>
    </div>
  );
